...
network_topology="physdev-to-vm" # two bridges, each with one physdev and one dpdkvhost
...
screen -dmS ovs \
sudo su -g qemu -c "umask 002; $prefix/sbin/ovs-vswitchd --dpdk $cuse_dev_opt -c 0x1 -n 3 --socket-mem 1024,1024 -- unix:$DB_SOCK --pidfile --log-file=$prefix/var/log/openvswitch/ovs-vswitchd.log 2>&1 >$prefix/var/log/openvswitch/ovs-launch.txt" 
...
## ovs dpdk thread pinning
# create 4 DPDK polling threads on cpus 4,6,8,10 (change if node0 cpus are on 0-7)
ovs-vsctl set Open_vSwitch . other_config:pmd-cpu-mask=550
